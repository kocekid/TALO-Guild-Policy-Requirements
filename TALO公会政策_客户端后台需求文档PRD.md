...
| 条件 | 规则 |
|---|---|
| 主播存在可提现金币余额 | 个人余额或主播转入金额大于0；金币工资奖励余额不作为独立余额选择项 |
| 提现金额合法 | 必须先选择个人余额或主播转入金额；金额必须大于0且不超过所选余额池可用余额 |
| 汇率可用 | 当前金币提现汇率配置有效 |
...
| 字段 | 说明 | 数据来源 |
|---|---|---|
| 可提现金币金额 | 所选个人余额或所选主播转入金额对应的金币出口可申请金额 | 金币出口下的可申请金额 |
...
2. 若用户从“仅支持提现为金币的金额”弹窗点击“去提现”进入本页，页面默认使用金币提现出口，但仍必须选择 `个人余额` 或 `主播转入金额` 作为本次扣减余额池。
...
| 场景 | 处理规则 |
|---|---|
| 只有金币工资奖励余额 | 不允许单独发起提现；提示需选择个人余额或主播转入金额作为本次金币提现余额池 |
...
| Reviewed | 已审核页签 | 仅展示 `Approved`、`Rejected` 状态记录 | 提现申请记录 |
...
3. 点击 `Reviewed` 仅展示已审核记录，包括 `Approved` 和 `Rejected`。
4. 点击编号旁复制图标，可复制提现编号。
...
| 状态分组 | `Waiting` 归入 `Under Review`；`Approved`、`Rejected` 归入 `Reviewed`；打款失败、已回滚等异常状态不混入本页 `Reviewed` 普通分组 |
...
| 提现金额合法 | 必须选择个人余额或主播转入金额；金额必须大于0且不超过所选余额池可用金额 |
| 汇率可用 | 当前金币提现汇率配置有效 |
...
| 查看原因 | 打开“仅支持提现为金币的金额”说明弹窗 | 页面操作 |
| 提现金额 | 本次申请金额 | 用户输入 |
| All Balance | 一键填入当前所选余额池的全部可用金额 | 页面操作 |
...
2. 若从“仅支持提现为金币的金额”弹窗点击“去提现”进入本页，页面默认使用金币提现出口，但仍必须选择 `个人余额` 或 `主播转入金额` 作为本次扣减余额池。
3. 公会长必须先选择 `个人余额` 或 `主播转入金额`；点击 All Balance 时，只填入当前所选余额池的全部可用金额。
4. 输入提现金额后，系统即时校验是否超过当前所选余额池可提现金额。
5. 页面同步展示预计到账金币数量。
...
- 金币提现只扣除本次所选的 `个人余额` 或 `主播转入金额`，不得跨余额池自动拼接扣减。
- 点击 All Balance 只代表“所选余额池全额”，不代表把个人余额、主播转入薪资余额、金币工资奖励余额合并清空。
- 主播转入薪资余额与个人薪资余额分开记录来源和扣减金额，不允许混成一个来源字段。
